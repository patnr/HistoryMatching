...
def field(ax, zz, **kwargs):
    ...
    # ax.imshow(Z[::-1])
    collections = ax.contourf(xx, yy, Z, **kwargs)
...
def oilfield(ax, ss, **kwargs):
    levels = linspace(0 -1e-7, 1 +1e-7, 11)
...
def animate1(saturation,production):
    ...
    if is_notebook_or_qt: plt.close() # https://stackoverflow.com/q/47138023

    tt = 1+arange(len(saturation))
    ...
    scat_handles = axs[1,1].scatter(*model.producers.T[:2], 24**2, 1-production[-1],
    ...
    ani = animation.FuncAnimation(fig, animate, len(tt), blit=False, interval=200)
...
def hists(fignum, samples, xlabel=""):
    ...
    ax.legend()
    ax.set(xlabel=xlabel, ylabel="Freq.")
